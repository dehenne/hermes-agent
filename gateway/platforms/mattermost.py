...
class MattermostAdapter(BasePlatformAdapter):
    ...
    async def _root_id_for_payload(
        ...
        """Return the Mattermost thread root_id for an outbound post.

        Gateway follow-up sends (progress, media, status) often do not carry
        a ``reply_to`` value, but they do carry ``metadata['thread_id']`` from
        the inbound event.  Use that trusted thread root first so follow-ups
        stay in the current Mattermost thread, including DM threads.  Fall
        back to resolving ``reply_to`` when no thread metadata is present.
        """
    ...
    async def send_multiple_images(
        ...
        for chunk_idx, chunk in enumerate(chunks):
            ...
            try:
                ...
                root_id = await self._root_id_for_payload(metadata=metadata)
                if root_id:
                    payload["root_id"] = root_id
                logger.info(
